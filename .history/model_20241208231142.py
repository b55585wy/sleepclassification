...
class SingleSalientModel(models.Model):
    ...
    def init_model(self, input: KerasTensor = None) -> (list, list):
        ...
        x = create_u_encoder(inputs, self.filters[0], self.kernel_size, 
                           self.pooling_sizes[0], self.u_inner_filter,
                           self.u_depths[0], "single_model", 1,
                           self.padding, self.activation)
        
        x = layers.Conv2D(self.filters[0], (1, 1), activation=self.activation,
                         padding=self.padding, name="channel_adjust")(x)
        
        x = layers.Reshape((self.sequence_length, self.sleep_epoch_len, 
                          self.filters[0]), name="reshape")(x)
        
        x = layers.Conv2D(self.filters[0], (1, 1), activation='tanh',
                         padding='same', name="final_conv")(x)
        x = layers.AveragePooling2D((1, self.sleep_epoch_len), 
                                   name="average_pool")(x)
        outputs = layers.Conv2D(self.num_classes, (self.kernel_size, 1),
                              padding=self.padding, activation='softmax',
...
class TwoSteamSalientModel(models.Model):
    ...
    def init_model(self, input: list = None) -> (list, list):
        ...
        :param input: 输入层列表，如果为None则创建新的输入层
        :return: 输入和输出层的列表
        """
        if input is None:
            # 创建两个输入层，一个用于EEG，一个用于EOG
            input = [
                layers.Input(shape=(self.sequence_length * self.sleep_epoch_length, 1, 1), name='EEG_input'),
                layers.Input(shape=(self.sequence_length * self.sleep_epoch_length, 1, 1), name='EOG_input')
            ]
        
        # 构建两个分支
        stream1 = self.build_branch(input[0], "EEG")  # EEG分支
        stream2 = self.build_branch(input[1], "EOG")  # EOG分支
        ...
        # 最终处理
        reshape = layers.Reshape((self.sequence_length, self.sleep_epoch_length, self.filters[0]))(merge)
