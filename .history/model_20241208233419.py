...
def build_two_stream_salient_model(params):
    ...
    # 创建输入层，注意输入维度
    total_length = sequence_length * sleep_epoch_length  # 20 * 3000 = 60000
    eeg_input = layers.Input(shape=(total_length, 1, 1), name='EEG_input')
    eog_input = layers.Input(shape=(total_length, 1, 1), name='EOG_input')
    ...
    # 确保维度正确
    # 首先计算当前特征图的大小
    current_length = total_length // np.prod(pooling_sizes[:2])  # 考虑池化的影响
    ...
    # 重塑维度前确保大小匹配
    print(f"Before reshape - merge shape: {merge.shape}")
    
    # 使用正确的reshape维度
    reshape = layers.Reshape((sequence_length, sleep_epoch_length, filters[0]),
                           name="reshape")(merge)
    print(f"After reshape - shape: {reshape.shape}")
