...
def preprocess(data, labels, param):
    """预处理数据
    Args:
        data: 原始数据列表，每个元素形状为 (N, 3000, 3)
        labels: 标签列表，每个元素形状为 (N,)
        param: 预处理参数字典
    Returns:
        x: 预处理后的数据，形状为 (M, sequence_epochs, 3000, 3)
        y: 预处理后的标签，形状为 (M, sequence_epochs)
    """
    ...
    for idx, (d, l) in enumerate(zip(data, labels)):
        try:
            ...
            # d shape: (1092, 3000, 3, 1, 1)
            # l shape: (1092,)
            
            # 确保数据长度是sequence_epochs的整数倍
            n_samples = d.shape[0]  # 片段数量
            ...
            # 重塑数据为序列
            # 从 (N, 3000, 3, 1, 1) 转换为 (N/sequence_epochs, sequence_epochs, 3000, 3, 1, 1)
