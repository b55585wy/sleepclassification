...
def build_branch(input_tensor, filters, kernel_size, pooling_sizes, 
    ...
    # 首先将6D输入重塑为4D
    # 从 [batch, 20, 3000, 3, 1, 16] 转换为 [batch, 60000, 1, 16]
    input_shape = input_tensor.shape
    if len(input_shape) == 6:
        batch_size = input_shape[0]
        total_time_steps = input_shape[1] * input_shape[2]  # 20 * 3000
        channels = input_shape[-1]
        input_tensor = layers.Reshape(
            (total_time_steps, 1, channels),
            name=f"{l_name}_initial_reshape"
        )(input_tensor)
    
    print(f"After reshape: {input_tensor.shape}")
...
def build_two_stream_salient_model(params):
    ...
    eeg_input = layers.Input(shape=(sequence_length * sleep_epoch_length, 1, 1),
                            name='EEG_input')  # [batch, 60000, 1, 1]
    eog_input = layers.Input(shape=(sequence_length * sleep_epoch_length, 1, 1),
                            name='EOG_input')  # [batch, 60000, 1, 1]

    print(f"Input shapes - EEG: {eeg_input.shape}, EOG: {eog_input.shape}")
