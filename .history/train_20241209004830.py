...
from model import SingleSalientModel, TwoSteamSalientModelWrapper
...
def build_and_compile_model(modal, hyper_params):
    ...
    if modal == 0:
        ...
        model = TwoSteamSalientModelWrapper(hyper_params).model
...
def train(args, hyper_params):
    ...
    # 加载数据
    print("Loading data...")
    data, labels = load_npz_files(data_dir)
    
    # 数据预处理
    print("Preprocessing data...")
    x_train, y_train = preprocess(data, labels, hyper_params['preprocess'])
