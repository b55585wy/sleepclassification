...
def create_u_encoder(inputs, filters, kernel_size, pooling_size, middle_layer_filter,
    ...
    """创建U型编码器
    Args:
        inputs: 输入张量，形状为 [None, 20, 3000, 3, 1, channels]
        filters: 卷积核数量
        kernel_size: 卷积核大小
        pooling_size: 池化大小
    ...
    # 重塑为4D张量 [batch*20, 3000, 3, channels]
    reshaped = layers.Reshape(
        (-1, time_steps, channels, input_shape[-1]),
        name=f"{l_name}_reshape_in"
    )(inputs)
    ...
    # 重塑回6D张量 [batch, 20, new_time, 3, 1, filters]
    final_shape = middle.shape
    output = layers.Reshape(
        (-1, seq_len, final_shape[1], channels, 1, final_shape[-1]),
        name=f"{l_name}_reshape_out"
    )(middle)
    
    print(f"Encoder {idx} - Output shape: {output.shape}")
    return output
